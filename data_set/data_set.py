...
class TestDataset:
    def __init__(self, config: datasets.DatasetConfig, num_pairs: int = 5, shuffle: bool = True):
        self.pairs = config.test_pairs
        self.tokens_config = TokenConfig()
        self.codec = build_codec()
        self.state = NoteDecodingState()
        self.vocabularies = TokensVocabulary(self.codec.num_classes)

    def __getitem__(self, n):
        pair = self.pairs[n]
        items = test_midi_and_audio_to_tokens(pair, spectrograms.SpectrogramConfig(), self.codec, self.tokens_config,
                                              self.vocabularies)
        return items
