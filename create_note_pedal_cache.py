...
from data_process.preprocess import tokenize_note_pedal, split_data
from data_process.vocabulary import build_codec, TokensVocabulary


def create_note_pedal_cache():
    ...
    for pair in pairs:
        ...
        for split_data_item in split_data_items:
            path = os.path.join(config.cache_split_note_pedal_path, str(cnt) + ".pt")
            # print(path)
